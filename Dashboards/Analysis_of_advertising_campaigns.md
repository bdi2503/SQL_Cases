...
SELECT
    ads_campaign,
    round((sum(revenue) - 250000) / 250000 * 100, 2) as roi
FROM (SELECT * FROM groups WHERE ads_campaign != '-') as t1
JOIN (
    SELECT 
        order_id,
...
checks_orders as (  
    ...
    FROM (
        ...
        AND creation_time::date BETWEEN '2022-09-01' AND '2022-09-07') as a
    JOIN products USING(product_id)  
    GROUP BY order_id)

SELECT
    ads_campaign,
    round(avg(avg_revenue), 2) as avg_check
FROM (
    SELECT 
        ads_campaign,
        user_id,
    ...
    FROM (SELECT * FROM groups WHERE ads_campaign != '-') as t1
    LEFT JOIN checks_orders USING(order_id)
    GROUP BY ads_campaign, user_id) as t2
GROUP BY ads_campaign
...
SELECT
    ...
    round(count_user_id::decimal / max(count_user_id) OVER(partition by ads_campaign, start_date), 2) as retention
FROM (    
    SELECT
        ads_campaign,
        start_date,
    ...
    FROM (   
        SELECT 
            ...
            END AS ads_campaign
        FROM user_actions
        ORDER BY user_id, date) as t1
    WHERE ads_campaign != '-'
    GROUP BY ads_campaign, start_date, date) as t2
